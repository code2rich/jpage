...
app.get('/api/content-templates/scenes', requireAuth, async (req, res) => {
...
app.get('/api/content-templates/:id/content', requireAuth, async (req, res) => {
  try {
...
app.post('/api/content-templates', requireAuth, async (req, res) => {
...
app.put('/api/content-templates/:id', requireAuth, async (req, res) => {
  try {
...
app.delete('/api/content-templates/:id', requireAuth, async (req, res) => {
  try {
    const t = await dbGet('SELECT id, uploaded_by FROM content_templates WHERE id = ?', [req.params.id]);
    if (!t) return res.status(404).json({ error: '模板不存在' });
    if (req.userRole !== 'admin' && t.uploaded_by !== req.userId) return res.status(403).json({ error: '无权操作' });
...
app.post('/api/content-templates/:id/use', requireAuth, async (req, res) => {
  try {
