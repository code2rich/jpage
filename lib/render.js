...
async function renderFile(res, file) {
  ...
  if (file.is_bundle) {
    ...
    try {
      ...
      const assetBasePath = file.asset_base_path || ('/api/files/' + file.id + '/asset/');
      const baseTag = '<base href="' + assetBasePath + entryDirPart + '">';
      ...
      if (/<head>/i.test(content)) {
        ...
        content = '<!doctype html><html><head><meta charset="UTF-8">' + baseTag + '</head><body>' + content + '</body></html>';
      }
...
// 内容模板公开短链 /t/:key 渲染。
// 与文件渲染对齐：Markdown 走 marked + 默认模板；HTML 注入 charset 后下发宽松 CSP。
async function renderTemplateContent(res, t) {
  if (t.file_type === 'markdown') {
    const html = marked.parse(t.content, { gfm: true, breaks: false, async: false })
      .replace(/<pre><code class="hljs language-mermaid">([\s\S]*?)<\/code><\/pre>/g,
        (_, code) => `<pre class="mermaid">${code}</pre>`);
    const tpl = templateCache['default'];
    const fullHtml = applyTemplate(tpl, t.title, html, 'github');
    return sendMarkdownHtml(res, fullHtml);
  }
  return sendHtmlDoc(res, ensureCharset(t.content));
}
