...
const { now, generateShareKey, currentUserId, clientIp, resolveUploadSource } = require('./util');
...
const { checkFileOwnership } = require('./middleware/files');
...
const zipIdentityLocks = new Map();

async function withZipIdentityLock(key, action) {
  const previous = zipIdentityLocks.get(key) || Promise.resolve();
  let release;
  const current = new Promise(resolve => { release = resolve; });
  zipIdentityLocks.set(key, current);
  await previous;
  try {
    return await action();
  } finally {
    release();
    if (zipIdentityLocks.get(key) === current) zipIdentityLocks.delete(key);
  }
}
...
async function handleZipUpload(req, res, zipBuffer, { targetFileId } = {}) {
  try {
    ...
    if (classification.type === 'bundle') {
      const dirName = generateStoredName();
      const bundleDir = path.join(UPLOAD_DIR, dirName);
      let extraction;
      try {
        extraction = await extractEntries(zip, entries, bundleDir);
      } catch (e) {
        await removeStoredObject(dirName, true).catch(() => {});
        throw e;
      }
      const totalSize = extraction.totalSize;
      ...
      const lockKey = targetFileId
        ? `target:${targetFileId}`
        : `name:${userId}:${originalName}`;

      return withZipIdentityLock(lockKey, async () => {
        let existing;
        if (targetFileId) {
          existing = await dbGet('SELECT * FROM files WHERE id = ?', [targetFileId]);
          if (!existing) {
            await removeStoredObject(dirName, true);
            return res.status(404).json({ error: '文件不存在' });
          }
          if (!checkFileOwnership(req, existing)) {
            await removeStoredObject(dirName, true);
          ...
          existing = await dbGet(
            `SELECT * FROM files
              WHERE original_name = ? AND uploaded_by = ?
              ORDER BY id ASC
              LIMIT 1`,
            [originalName, userId]
          );
        ...
        if (existing && !existing.is_bundle) {
          await removeStoredObject(dirName, true);
          return res.status(400).json({ error: '文件类型不匹配：ZIP 网站包只能覆盖网站包' });
        }
          ...
          try {
            const { version } = await backupAndApplyVersion(
              existing,
              {
                ...
                fileType,
                isBundle: true,
                entryPath: classification.entryFile,
              },
              existing.uploaded_by,
              source,
              userId
            );
            logger.audit('file.overwrite', {
              fileId: existing.id,
        ...
        try {
          ...
          await addUserStorage(userId, totalSize);
    ...
    if (targetFileId) {
      return res.status(400).json({ error: '批量 ZIP 不能覆盖单个文件，请上传网站包 ZIP' });
    }

    const baseNames = classification.files.map(entry => path.basename(entry.name));
    const duplicateBaseNames = [...new Set(baseNames.filter((name, index) => baseNames.indexOf(name) !== index))];
    if (duplicateBaseNames.length > 0) {
      return res.status(400).json({ error: 'ZIP 内存在同名文件: ' + duplicateBaseNames.join(', ') });
    }
    ...
    for (const entry of classification.files) {
      ...
      try {
        ...
        await withZipIdentityLock(`name:${userId}:${baseName}`, async () => {
          ...
          if (existing && existing.is_bundle) {
            throw userError('文件类型不匹配：批量 ZIP 内文件不能覆盖网站包');
          }

          if (existing) {
            const { version } = await backupAndApplyVersion(
              existing,
              { storedName, size: buf.length, fileType, isBundle: false, entryPath: null },
              existing.uploaded_by,
              source,
              userId
            );
            const currentStoredName = storedName;
            // 数据库已切换到新文件；后续非关键索引失败时不能把当前文件当作临时文件删除。
            storedName = null;
            if (isFtsIndexable(fileType, currentStoredName)) {
              try {
                indexFileContent(existing.id, currentStoredName);
              } catch (e) {
                logger.error({ type: 'app', action: 'zip.batch.index', fileId: existing.id, error: e.message });
              }
            }
            results.push({
              id: existing.id,
          ...
          await addUserStorage(userId, buf.length);
          const currentStoredName = storedName;
          storedName = null;
          if (isFtsIndexable(fileType, currentStoredName)) {
            try {
              indexFileContent(dbResult.lastID, currentStoredName);
            } catch (e) {
              logger.error({ type: 'app', action: 'zip.batch.index', fileId: dbResult.lastID, error: e.message });
            }
          }
          const shareKey = await dbGet('SELECT share_key FROM files WHERE id = ?', [dbResult.lastID]).then(row => row?.share_key);
          results.push({
            id: dbResult.lastID,
            original_name: baseName,
            file_type: fileType,
        ...
        if (storedName) await removeStoredObject(storedName).catch(() => {});
