...
除非用户要的是**幻灯片 / PPT / 演示文稿**（那走 `jpage-presentation` 的 Bundle 模式），否则**一律生成单个自包含 HTML 文件**：CSS 用 `<style>` 内联、JS 用 `<script>` 内联、图片用 data URI 或在线 URL，全部塞进一个 `.html` 里。

不要把页面拆成 `index.html + style.css + app.js` 再打 ZIP——单文件在即页里预览最稳、分享最简单，没有资源相对路径的问题。普通报告、仪表板、落地页、可视化、简历、邮件模板都该是单文件。

只有这几种情况才用多文件 / ZIP：
- **幻灯片**：reveal.js 引擎 ~85KB，必须 Bundle（见 `jpage-presentation`）
- **批量上传**：用户一次给了多个相互独立的页面要一起传
- **大型本地资源**：确有无法内联的二进制（字体、视频、大量高清图）
